...
class ParakeetRNNTLoss(nn.Module):
    ...
    def _compute_forward_prob(
        ...
        terminal_log_probs = torch.stack(
            [
                log_alpha[b, act_lens[b] - 1, label_lens[b]] + log_probs[b, act_lens[b] - 1, label_lens[b], self.blank]
                for b in range(B)
            ]
        )
...
class ParakeetTDTLoss(nn.Module):
    ...
    def forward(
        ...
        rnnt_forward_log_probs = self._rnnt_loss._compute_forward_prob(log_label_acts, labels, act_lens, label_lens)
    ...
    def _greedy_decode(
        ...
        for time_idx in range(max_out_len):
            ...
            while not_blank and symbols_added < self.config.max_symbols_per_step:
                if time_idx == 0 and symbols_added == 0 and all(state[0].abs().sum() == 0 for _ in [None]):
    ...
    def _greedy_decode(
        ...
        while (time_indices < encoder_output_length).any():
            ...
            while not frame_done.all() and symbols_added < self.config.max_symbols_per_step:
                ...
                safe_dur = torch.where(
                    k_is_blank & (predicted_dur == 0), torch.ones_like(predicted_dur), predicted_dur
                )
